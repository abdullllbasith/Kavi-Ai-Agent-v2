...
export type ToolSecurityPolicy = { requiresConfirmation?: boolean; authorize?: (input: unknown, state: AgentState) => Promise<boolean> | boolean; validate?: (input: unknown, state: AgentState) => void };
export class ToolRegistry {
  private readonly tools = new Map<string, ToolDefinition>(); private readonly policies = new Map<string, ToolSecurityPolicy>();
  register<TInput, TOutput>(tool: ToolDefinition<TInput, TOutput>, policy: ToolSecurityPolicy = {}): void { if (this.tools.has(tool.name)) throw new Error(`Tool already registered: ${tool.name}`); this.tools.set(tool.name, tool as ToolDefinition); this.policies.set(tool.name, policy); }
  get(name: string): ToolDefinition { const tool = this.tools.get(name); if (!tool) throw new Error(`Unknown tool: ${name}`); return tool; }
