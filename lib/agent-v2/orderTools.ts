...
export type OrderExecutor = {
  ...
  authorizeOrder?: (userId: string, orderId: string) => Promise<boolean>;
};
...
function authorized(executor: OrderExecutor, input: unknown, state: AgentState): Promise<boolean> | boolean {
  ...
  const data = requireRecord(input, 'order');
  const orderId = requireString(data, 'orderId', 'order');
  const userId = typeof state.constraints.userId === 'string' ? state.constraints.userId : '';
  if (!userId) return false;
  return executor.authorizeOrder(userId, orderId);
}

export function registerOrderTools(executor: OrderExecutor): ToolDefinition[] {
  const tools: ToolDefinition[] = [];
  if (executor.getOrderStatus) {
    tools.push(createCommerceReadTool('get_order_status', 'Read the current status of an order owned by the current user.', async (input, state) => {
      if (!(await authorized(executor, input, state))) throw new Error('Order authorization denied.');
  ...
  if (executor.getDeliveryEstimate) {
    tools.push(createCommerceReadTool('get_delivery_estimate', 'Read delivery information for an order owned by the current user.', async (input, state) => {
      if (!(await authorized(executor, input, state))) throw new Error('Order authorization denied.');
