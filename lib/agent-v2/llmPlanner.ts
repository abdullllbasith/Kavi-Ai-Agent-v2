...
function summarizeState(state: AgentState): string {
  return JSON.stringify({ userMessage: state.userMessage, goal: state.goal, constraints: state.constraints, plan: state.plan, observations: state.observations.slice(-6), actions: state.actions.slice(-6).map((a) => ({ tool: a.tool, status: a.status, error: a.error })), iteration: state.iteration, lastEvaluation: state.lastEvaluation });
}
...
export function createLLMEvaluator(call: LLMStructuredCall) {
  return async (state: AgentState, toolResult: unknown, tools: ToolRegistry): Promise<EvaluationDecision> => {
    const availableTools = tools.list().map((tool) => tool.name);
    const system = `You are Kavi's goal evaluator. Determine whether the user's goal is actually satisfied by observed evidence. Be conservative: missing evidence is not success. If not satisfied, identify what is missing and suggest one next action using only the available tools. Do not invent facts.`;
    ...
    return validateEvaluationDecision(raw, availableTools);
  };
}
