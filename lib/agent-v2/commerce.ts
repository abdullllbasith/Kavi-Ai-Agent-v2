...
export type CommerceExecutor = { addToCart?: (input: { productId: string; quantity: number }) => Promise<unknown>; checkout?: (input: Record<string, unknown>) => Promise<unknown>; cancelOrder?: (input: { orderId: string }) => Promise<unknown>; authorize?: (userId: string, action: CommerceAction, input: Record<string, unknown>) => Promise<boolean> | boolean };
