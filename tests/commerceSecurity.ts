...
const state: AgentState = { turnId: 'security-test', status: 'acting', userMessage: 'buy', goal: null, constraints: { userId: 'user-1' }, plan: [], observations: [], actions: [], iteration: 1, maxIterations: 3 };
let executed = false;
...
const confirmed = { ...state, constraints: { ...state.constraints, confirmation: { action: 'add_to_cart', fingerprint: createActionFingerprint('add_to_cart', { productId: 'p1', quantity: 1 }), confirmedAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 60_000).toISOString() } } };
...
const unauthorized = { ...state, constraints: { ...state.constraints, userId: 'user-2', confirmation: { action: 'add_to_cart', fingerprint: createActionFingerprint('add_to_cart', { productId: 'p1', quantity: 1 }), confirmedAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 60_000).toISOString() } } };
