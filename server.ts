...
function allowed(req: IncomingMessage, key: string) { const now = Date.now(); const current = rate.get(key); if (!current || now - current.window >= WINDOW_MS) { rate.set(key, { window: now, count: 1 }); return true; } current.count += 1; return current.count <= MAX_REQUESTS; }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/session') { const session = createSession(); setSession(res, session.id); return json(res, 201, { userId: session.userId }); }
    const session = getSession(req);
    if (url.pathname.startsWith('/api/')) {
      if (!session) return json(res, 401, { error: 'Session required.' });
      if (!allowed(req, session.userId)) return json(res, 429, { error: 'Rate limit exceeded. Try again shortly.' });
    }
    if (req.method === 'POST' && url.pathname === '/api/chat') {
      ...
      const result = await runKaviV2(message, { userId: session!.userId, llm });
      return json(res, 200, { state: result.state, language: result.language, responsePolicy: result.responsePolicy });
    }
    if (req.method === 'GET') return serveStatic(req, res);
    return json(res, 405, { error: 'Method not allowed.' });
  } catch (error) { console.error('[kavi]', error); json(res, 500, { error: error instanceof Error ? error.message : 'Internal server error.' }); }
});
