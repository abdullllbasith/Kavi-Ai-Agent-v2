...
$('#newChat').addEventListener('click', () => { $('#messages').innerHTML = ''; toast('Started a new Kavi conversation'); });
...
async function sendMessage() {
  ...
  appendMessage('user', value); input.value = '';
  setAgentBusy(true);
  try {
    await ensureSession();
    ...
    appendMessage('assistant', error instanceof Error ? error.message : 'Kavi could not process the request.');
    toast('Request failed safely');
...
function renderAgentState(state) {
  if (state.goal) {
    const title = document.querySelector('.goal-content strong');
    if (title) title.textContent = state.goal.objective;
    const criteria = document.querySelector('.criteria');
    if (criteria) criteria.innerHTML = state.goal.successCriteria.map((x, i) => `<span>${i < state.observations.length ? '✓' : '●'} ${escapeHtml(x)}</span>`).join('');
  }
...
function updateContext(state) {
  ...
  (state.goal?.successCriteria || []).forEach((criterion, i) => { if (checks[i]) { checks[i].classList.toggle('done', i < state.observations.length); checks[i].classList.toggle('active', i === state.observations.length); checks[i].lastChild.textContent = ` ${criterion}`; } });
}
